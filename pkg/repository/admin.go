...
func (ad *adminRepository) UpdateBlockUserByID(user domain.Users) error {
	...
	err := ad.DB.Exec("update users set blocked = true where id = ?", user.ID).Error
	if err != nil {
...
func (ad *adminRepository) UpdateUnBlockUserByID(user domain.Users) error {

	err := ad.DB.Exec("update users set blocked = false where id = ?", user.ID).Error
	if err != nil {
		fmt.Println("Error updating user:", err)
		return err
	}
